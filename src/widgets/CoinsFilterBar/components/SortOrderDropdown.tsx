...
export function SortOrderDropdown() {
    ...
    const { sortOrder: currentSortOrder } = useAppSelector(
        ...
        Object.values(CoinsSortOrder).forEach((sortOrder) => {
            sortOrders.push(
                <option
                    value={sortOrder}
                    key={sortOrder}
                    className='px-2 py-1 outline-none border hover:bg-none'>
                    {sortOrder}
                </option>,
            );
        });
    ...
    return (
        <div className='w-max flex flex-col items-start gap-2'>
            ...
            <select
                ...
                onChange={(e) =>
                    dispatch(setSortOrder(e.target.value as CoinsSortOrder))
                }>
